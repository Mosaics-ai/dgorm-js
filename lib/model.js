...
class Model {
     ...
     * @returns Promise<any> Entire object will be returned using params
     */
    async create_batch(data, params) {
        let mutations = [];
        ...
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                if (Array.isArray(mutation)) {
                    ...
                    const data = await this._method('uid', uids, params);
                    return resolve(data);
                }
                else {
                    const _uid = _uids.values().next().value;
                    const data = await this._method('uid', _uid, params);
